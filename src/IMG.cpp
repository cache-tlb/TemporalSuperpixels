...
void IMG::readFromStruct(IMG_STRUCT *img_struct_ptr) {
    // read in the variables from the structure
    xdim = img_struct_ptr->xdim;        // xdim the cv::Mat row number, and image height
    ydim = img_struct_ptr->ydim;
    N = xdim*ydim;
    K = img_struct_ptr->K;
    data = getArrayInput<double>(img_struct_ptr->data);
    T4Table = getArrayInput<double>(img_struct_ptr->T4Table);

    log_alpha = img_struct_ptr->log_alpha;
    log_beta = img_struct_ptr->log_beta;
    area = img_struct_ptr->area;
    area_var = img_struct_ptr->area_var;
    log_area_var = log(area_var);
    max_UID = img_struct_ptr->max_uid;
    // ::debug() << log_alpha << log_beta << area << area_var << log_area_var << max_UID;

    // create temporary space for variables that we will change
    int* temp_label = getArrayInput<int>(img_struct_ptr->label);
    ...
    new_pos = NormalD(2, getArrayInput<double>(img_struct_ptr->hyper.p_theta), getArrayInput<double>(img_struct_ptr->hyper.p_Delta), true);
    new_app = NormalD(3, getArrayInput<double>(img_struct_ptr->hyper.a_theta), getArrayInput<double>(img_struct_ptr->hyper.a_Delta), true);
    SP_new = new SP(new_pos, new_app, 0, false);
    
    // int num_old = mxGetNumberOfElements(tmp_SP);
    int num_old = img_struct_ptr->SP.size();

    SP_arr = allocate_memory<SP*>(N);
    SP_old = allocate_memory<bool>(N, false);

    std::vector<IMG_STRUCT::SP_STRUCT> &sp = img_struct_ptr->SP;

    for (int i = 0; i < num_old; i++) {
        if (!sp[i].old) {
            if (img_struct_ptr->SP[i].UID < 0) {
                SP_arr[i] = new SP(new_pos, new_app, max_UID++, false);
            }
            else 
                SP_arr[i] = new SP(new_pos, new_app, sp[i].UID, false);
            SP_old[i] = false;
        } else {
            NormalD tmp_pos = NormalD(2, getArrayInput<double>(sp[i].v), getArrayInput<double>(sp[i].p_theta), getArrayInput<double>(sp[i].p_Delta), false);
            NormalD tmp_app = NormalD(3, getArrayInput<double>(sp[i].a_theta), getArrayInput<double>(sp[i].a_Delta), false);
            unsigned long UID = sp[i].UID;
            arr(double) prev_v = getArrayInput<double>(sp[i].prev_v);
            SP_arr[i] = new SP(tmp_pos, tmp_app, UID, true, prev_v);
            SP_arr[i]->set_mean_pos(getArrayInput<double>(sp[i].p_mu));
            SP_arr[i]->set_mean_app(getArrayInput<double>(sp[i].a_mu));
            SP_old[i] = true;
        }
    }
...
//     ::debug() << "K:" << K << "N:" << N << "xdim:" << xdim << "ydim" << ydim; 
//     for (int i = (376*200+100)*5; i < (376*200+100)*5+20; i++) {
//         ::debug()<<data[i];
//     }
//     ::debug() << "";
//     for (int i = 500*200*5; i < 500*200*5+20; i++) {
//         ::debug()<<data[i];
//     }
    // ::debug() << "here" << img_struct_ptr->SP_changed.rows << img_struct_ptr->SP_changed.cols << N;
    SP_changed = allocate_memory<bool>(N, true);
    arr(int) tmp_SP_changed = getArrayInput<int>(img_struct_ptr->SP_changed);
    for (int i = 0; i < N; i++)
        SP_changed[i] = tmp_SP_changed[i];

    boundary_mask = getArrayInput<int>(img_struct_ptr->boundary_mask);          // may set to bool later
    dummy_log_prob = img_struct_ptr->dummy_log_prob;
    // ::debug() << "dummy_log_prob:" << dummy_log_prob;

    // read in the previous labels
    if (!img_struct_ptr->prev_label.empty()) {
        prev_label = getArrayInput<int>(img_struct_ptr->prev_label);
        prev_app_mean = getArrayInput<double>(img_struct_ptr->prev_app_mean);
        prev_pos_mean = getArrayInput<double>(img_struct_ptr->prev_pos_mean);
        prev_K = img_struct_ptr->prev_K;
        prev_covariance = getArrayInput<double>(img_struct_ptr->prev_covariance);
        prev_precision = getArrayInput<double>(img_struct_ptr->prev_precision);
        alive_dead_changed = img_struct_ptr->alive_dead_changed;
        if (alive_dead_changed) {
            ...
            arr(double) tempSxy = getArrayInput<double>(img_struct_ptr->Sxy);
            arr(double) tempSyy = getArrayInput<double>(img_struct_ptr->Syy);
            arr(double) tempSxySyy = getArrayInput<double>(img_struct_ptr->SxySyy);
            num_alive = img_struct_ptr->Syy.rows;                                // !! m is rows
        ...
        temp_still_alive = allocate_memory<bool>(prev_K, false);
        all2alive = allocate_memory<int>(prev_K ,0);
        alive2all = allocate_memory<int>(prev_K, 0);
        all2dead = allocate_memory<int>(prev_K, 0);
        dead2all = allocate_memory<int>(prev_K, 0);
    } else { // first frame
        prev_label = NULL;
        prev_app_mean = NULL;
        prev_pos_mean = NULL;
...
void IMG::writeToStruct(IMG_STRUCT *img_struct_ptr, double *new_E, double *K_out, cv::Mat *label_out, std::vector<IMG_STRUCT::SP_STRUCT> *sp_out, cv::Mat *sp_changed_out, int *max_UID_out, bool *alive_dead_changed_out, cv::Mat *Sxy_out, cv::Mat *Syy_out, cv::Mat *SxySyy_out) {
    // left: IMG.K, IMG.label, IMG.SP, IMG.SP_changed, IMG.max_UID, IMG.alive_dead_changed, IMG.Sxy,IMG.Syy,IMG.SxySyy, newE
    // right[0]: IMG
    assert(xdim*ydim==N);
    if (K_out) (*K_out) = K;
    if (label_out) {
        if (label_out->rows*label_out->cols != N)
            label_out->create(ydim, xdim, CV_64F);
        memcpy((int*)(label_out->data), label, sizeof(int)*N);
    }
    ...
    if (sp_out){
        ...
        for (int i = 0; i < K; i++) {
            if (SP_arr[i] != NULL) {
                IMG_STRUCT::SP_STRUCT &sp = (*sp_out)[i];
                writeVectorToCvVec(sp.p_theta,   SP_arr[i]->get_theta_pos());  //mxWriteField(tmp_SP,i,"p_theta",mxWriteVector2<double>(1,2,    SP_arr[i]->get_theta_pos()));
                writeVectorToCvVec(sp.v,         SP_arr[i]->get_flow());       //mxWriteField(tmp_SP,i,"v",      mxWriteVector2<double>(1,2,    SP_arr[i]->get_flow()));
                writeVectorToCvVec(sp.p_Delta,   SP_arr[i]->get_Delta_pos());  //mxWriteField(tmp_SP,i,"p_Delta",mxWriteVector2<double>(1,2,    SP_arr[i]->get_Delta_pos()));
                writeVectorToCvVec(sp.p_mu,      SP_arr[i]->get_mean_pos());   //mxWriteField(tmp_SP,i,"p_mu",   mxWriteVector2<double>(1,2,    SP_arr[i]->get_mean_pos()));
                writeVectorToCvVec(sp.a_Delta,   SP_arr[i]->get_Delta_app());  //mxWriteField(tmp_SP,i,"a_Delta",mxWriteVector2<double>(1,3,    SP_arr[i]->get_Delta_app()));
                writeVectorToCvVec(sp.a_theta,   SP_arr[i]->get_theta_app());  //mxWriteField(tmp_SP,i,"a_theta",mxWriteVector2<double>(1,3,    SP_arr[i]->get_theta_app()));
                writeVectorToCvVec(sp.a_mu,      SP_arr[i]->get_mean_app());   //mxWriteField(tmp_SP,i,"a_mu",   mxWriteVector2<double>(1,3,    SP_arr[i]->get_mean_app()));
                sp.N = SP_arr[i]->get_N();                                     //mxWriteField(tmp_SP,i,"N",      mxWriteScalar(        (double)SP_arr[i]->get_N()));
                sp.UID = SP_arr[i]->get_UID();                                 //mxWriteField(tmp_SP,i,"UID",    mxWriteScalar(                SP_arr[i]->get_UID()));
                sp.old = SP_old[i];                                            //mxWriteField(tmp_SP,i,"old",    mxWriteScalar(                SP_old[i]));
                writeVectorToCvVec(sp.prev_v,    SP_arr[i]->get_prev_v());     //mxWriteField(tmp_SP,i,"prev_v", mxWriteVector2<double>(1,2,    SP_arr[i]->get_prev_v()));
            } else if (SP_old[i]) {
                printf("AHHHHHHHHHH!\n");
            }
        }
    ...
    if (sp_changed_out){
        if (sp_changed_out->rows*sp_changed_out->cols != N) 
            sp_changed_out->create(1, N, CV_32S);
        for (int i = 0; i < N; i++)
            sp_changed_out->at<int>(i) = (SP_changed[i] && SP_arr[i]!=NULL && !SP_arr[i]->isempty()) ? 1 : 0;
    }
    
    if (max_UID_out)
        (*max_UID_out) = max_UID;
    if (alive_dead_changed_out)
        (*alive_dead_changed_out) = alive_dead_changed;

    if (Sxy_out) {
        if (Sxy_out->rows*Sxy_out->cols != num_alive*prev_K)
            Sxy_out->create(num_alive,prev_K, CV_64F);
        memcpy((void*)Sxy_out->data, Sxy, sizeof(double)*num_alive*prev_K);
    }

    if (Syy_out) {
        if (Syy_out->rows*Syy_out->cols != num_alive*num_alive)
            Syy_out->create(num_alive,num_alive, CV_64F);
        memcpy((void*)Syy_out->data, Syy, sizeof(double)*num_alive*num_alive);
    }

    if (SxySyy_out) {
        if (SxySyy_out->rows*SxySyy_out->cols != num_alive*prev_K)
            SxySyy_out->create(num_alive,prev_K,CV_64F);
        memcpy((void*)SxySyy_out->data, SxySyy, sizeof(double)*num_alive*prev_K);
    }

    double E = U_calc_energy();
    if (new_E) *new_E = E;
}
...
void IMG::U_print_neighbors(int k)
{
   ...
   printf("%d 's NB (%d): ",k,SP_arr[k]->neighbors.getLength());
   while (neighborNode!=NULL)
   {
      printf("%d   ",neighborNode->getData().first);
...
double IMG::link_cost(int ko, int kn)
{
   ...
   {
      printf("Can't link a dead SP to a new SP!");
   }
   return logprob;
}
...
double IMG::move_local_calc_delta(int index, int new_k, bool add, double& max_prob, int& max_k)
{
   ...
   if (new_k<0)
   {
      prob = boundary_mask[index] ? -1.0*std::numeric_limits<double>::max() : dummy_log_prob;
   }
   else
   {
      SP* temp;
...
void IMG::move_merge_SP_propose(int k,arr(bool) neighbors,double &max_E,int &max_k)
{
   ...
   if (max_k==-1)
      printf("find_bestnb: No neighbour found\n");
}
void IMG::move_merge_SP_propose_region(int k,arr(bool) neighbors,linkedList<int> &check_labels,double &max_E,int &max_k)
{
   ...
   if (max_k==-1)
   {
      neighborsLL.print();
      printf("k=%d\n", debug);
      printf("find_bestnb_region: No neighbour found\n");
   }
}
...
bool IMG::U_Kmeans_plusplus(int index, int *bbox, int num_SP, int numiter, int index2)
{
   ...
   if (num_SP!=2)
      printf("Trying to split into more than 2");
   ...
   if (!old_split1 && !old_split2) // both new
   {
      ...
      if(label[seed]!=index)
      {
         printf("SP_arr[index]->get_N()=%d\n", SP_arr[index]->get_N());
         printf("inconsistency about cluster label\n");
      }
      for( int n = 0; n < 5; n++ )
      ...
      for( int k = 0; k < num_SP; k++ )
      {
         ...
         else
         {
            if (old_split1 || old_split2)
               ...
               printf("one cluster removed... shouldn't happen\n");
         }
      }
   }
...
void IMG::U_connect_newSP(int *bbox,int num_SP){
   ...
   if (label_count>num_SP)
   {
      ...
      for(int i =0; i<label_count-num_SP; i++)
      {
         ...
         if(max_k==-1||max_E==-1)
            printf("the redundant SP doesn't merge within the range...\n");
      ...
      for(int i =0; i<num_SP; i++)
      {
         while(SP_arr[max_k]==NULL)
         {
            max_k++;
            if(max_k>N)
               printf("should be more nonempty SPs...\n");
         }
         if(max_k!=K+i)
         {
