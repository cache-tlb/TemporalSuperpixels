...
SP& SP::operator=(const SP& that)
{
    if (this != &that)
    {
        ...
        borders.clear();
        neighbors.clear();
        printf("SP ASSIGNMENT OPERATOR NOT COPYING LINKED LISTS AND UID COPIED!\n");
    }
    return *this;
}
...
{
    if (N<=0)
        printf("Trying to remove a pixel from an empty super pixel!");
...
void SP::update_neighbors_label_rem(int neighbor_label)
{
    ...
    if (found)
    {
        ...
        printf("Trying to remove a neighbor that was never added");
}
// --------------------------------------------------------------------------
