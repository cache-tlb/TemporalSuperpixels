...
public:
    ...
    virtual bool fromCvMat(const cv::Mat &im);
    virtual bool toCvMat(cv::Mat &im);
...
template <class T>
bool Image<T>::fromCvMat(const cv::Mat &im) {
    clear();
    if (ImageIO::fromCvMat(im, pData, imWidth, imHeight, nChannels)) {
        computeDimension();
        colorType = BGR;
        return true;
    }
...
template <class T>
bool Image<T>::toCvMat(cv::Mat &im) {
    return ImageIO::toCvMat(im, pData, imWidth, imHeight, nChannels);
}
