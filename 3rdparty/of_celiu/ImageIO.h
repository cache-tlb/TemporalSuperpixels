...
public:
    ...
    template <class T>
    static bool fromCvMat(const cv::Mat &im,T*& pImagePlane,int& width,int& height, int& nchannels);
    ...
    template <class T>
    static bool toCvMat(cv::Mat &im,const T* pImagePlane,int width,int height, int nchannels);
...
template <class T>
bool ImageIO::fromCvMat(const cv::Mat &im,T*& pImagePlane,int& width,int& height, int& nchannels) {
    // we only support three types of image information for now
    if(im.data == NULL) // if allocation fails
        return false;
    if(im.type()!= CV_8UC1 && im.type()!=CV_8UC3 && im.type()!=CV_8UC4)
        return false;
    width = im.size().width;
    height = im.size().height;
    nchannels = im.channels();
...
template <class T>
bool ImageIO::toCvMat(cv::Mat &im,const T* pImagePlane,int width,int height, int nchannels) {
    if (typeid(T)==typeid(float)) {
        switch(nchannels){
        case 1:
            im.create(height,  width, CV_32FC1); break;
        case 2:
            im.create(height,  width, CV_32FC2); break;
        case 3:
            im.create(height,  width, CV_32FC3); break;
        default:
            return false;
        }
        ...
        switch(nchannels){
        case 1:
            im.create(height,  width, CV_64FC1); break;
        case 2:
            im.create(height,  width, CV_64FC2); break;
        case 3:
            im.create(height,  width, CV_64FC3); break;
        default:
            return false;
        }
        ...
        switch (nchannels) {
        case 1:
            im.create(height,  width, CV_8UC1); break;
        case 2:
            im.create(height,  width, CV_8UC2); break;
        case 3:
            im.create(height,  width, CV_8UC3); break;
        default:
            return false;
        }
    } else {
        printf("ImageIO::toCvMat: unsupported type\n");
    }
    im.setTo(0);
    ...
    if(typeid(T) == typeid(unsigned char)) {
        ...
        for (int i = 0; i < height; i++) {
            int offset1 = i*width*nchannels;
            T *ptr = ((T*)im.data)+width*nchannels*i;
            for(int j = 0; j < width*nchannels; j++) {
                ptr[j] = pImagePlane[offset1+j];
            }
        }
    }
    return true;
